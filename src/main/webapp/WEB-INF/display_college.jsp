<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<!DOCTYPE html>
<html>
<head>
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta charset="utf-8">
<script src="https://kit.fontawesome.com/a076d05399.js"></script>
<meta name="viewport"
	content="width=device-width, initial-scale=1, shrink-to-fit=no">
<link rel="stylesheet"
	href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css"
	integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO"
	crossorigin="anonymous">
<link rel="stylesheet" href="css/styles.css">
<link rel="preconnect" href="https://fonts.gstatic.com">
<link
	href="https://fonts.googleapis.com/css2?family=Seaweed+Script&display=swap"
	rel="stylesheet">
<style>
@charset "ISO-8859-1";
footer {
	position: absolute;
	top: 600px;
	background: #ffffff;
	width: 100%;
}

.main-content {
	color: black;
	display: flex;
}

.main-content .box {
	flex-basis: 50%;
	padding: 10px 30px;
}

.box h2 {
	font-size: 1.5rem;
	font-weight: 600;
	text-transform: uppercase;
}

.box .footercontent {
	margin: 15px 0 0 0;
}

.l .footercontent .social {
	margin: 15px 0 0 0;
}

.l .footercontent .p {
	text-align: justify;
}

.l .footercontent .social a {
	padding: 0 2px;
}

.l .footercontent .social a span {
	height: 40px;
	width: 40px;
	background: #1a1a1a;
	line-height: 40px;
	text-align: center;
	font-size: 18px;
	border-radius: 10px;
	transition: 0.3s;
	color: white;
}

.l .footercontent .social a span:hover {
	background: #ff8000;
}

body {
	font-family: Arial, Helvetica, sans-serif;
	background-color: white;
	color: white;
}

form {
	border-radius: 20px;
	background-color: #f2f2f2;
	padding: 20px;
	width: 800px;
	height: 350px;
}

.content {
	max-width: 500px;
	margin: auto;
}

input {
	background-color: white;
	padding: 5px 30px;
	margin: 8px 0;
	border: 3px solid rgb(0, 0, 0);
	width: 100%;
}

button {
	background-color: orange;
	color: white;
	padding: 14px 20px;
	margin: 8px 0;
	border: none;
	cursor: pointer;
	width: 100%;
}

.vertical-menu {
	width: 290px;
	border-radius: 10px;
	padding: 12px;
}

.vertical-menu a {
	background-color: grey;
	color: white;
	display: block;
	margin: 8px 0;
	border-radius: 10px;
	padding: 12px;
	text-decoration: none;
}

.vertical-menu a:hover {
	background-color: #ff952b;
}

.vertical-menu a.active {
	background-color: #04AA6D;
	color: white;
}
</style>

<meta charset="ISO-8859-1">
<title>College Ranking Management System</title>
</head>
<body>
	<div class="container-fluid p-0">
		<nav class="navbar navbar-expand-lg my-navbar">
			<a class="navbar-brand" href="/studentHomepage"><img id="logo"
				src="data:image/png;base64,"
				alt="logo"></a>

			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav m-auto">
					<li class="nav-item active">


						<center>
							<p style="color: #ff952b; font-size: 30px;">College Ranking
								Management System</p>
						</center>
				</ul>
			</div>
		</nav>
		<br>
		<center>

			<c:if test="${not empty collegeList}">
				<c:forEach var="collegeList" items="${collegeList}">
					<form
						style="border: 3px solid #ffffff; background: rgba(0, 0, 0, 0.6); background: rgba(0, 0, 0, 0.6);">
						<div style="font-size: 30px; line-height: 100%;">
							<p>
								<b>${ collegeList.collegeName}</b> <br> <small
									style="font-size: 14px;">${ collegeList.location}</small>
							</p>

						</div>
						<table>
							<tr style="color: #ff952b; width: 80px;">
								<th style="width: 170px;"><b>OverAll Rank</b></th>
								<th style="width: 170px;"><b>Employment Rank</b></th>
								<th style="width: 170px;"><b>Research Rank</b></th>
								<th style="width: 170px;"><b>Eligibility</b></th>
							</tr>
							<tr>
								<td>${ collegeList.overallRank}</td>
								<td>${ collegeList.employmentRank}</td>
								<td>${ collegeList.researchRank}</td>
								<td>${ collegeList.minimumMarks}</td>
							</tr>
							<tr style="color: #ff952b; width: 80px;">
								<th style="width: 170px;"><b>Top Package</b></th>
								<th style="width: 170px;"><b>Fee</b></th>
								<th style="width: 170px;"><b>Placement Percentage</b></th>

								<th style="width: 170px;"><b>Students Per Faculty</b></th>
							</tr>
							<tr>
								<td>${ collegeList.topPackage}</td>
								<td>${ collegeList.fee}</td>
								<td>${ collegeList.placementPercentage}</td>
								<td>${ collegeList.numberOfStudentsPerFaculty}</td>
							</tr>
							<tr style="color: #ff952b; width: 80px;">
								<th style="width: 170px;"><b>Branches</b></th>
								<th style="width: 170px;"><b>Average Package</b></th>
								<th style="width: 170px;"><b>pass Percentage</b></th>
							</tr>
							<tr>
								<td>${ collegeList.branches}</td>
								<td>${ collegeList.averagePackage}</td>
								<td>${ collegeList.passPercentage}</td>
							</tr>

						</table>

						<button onClick="window.open('${collegeList.collegeWebsite }');"
							style="float: right; background-color: #ff952b; width: 200px;">College
							Website</button>
					</form>

					<br>


				</c:forEach>
			</c:if>
			<h3
				style="border: 3px solid white; background-color: rgba(0, 0, 0, 0.6); height: 60px; padding: 10px; width: 800px; border-radius: 20px; color: #ff952b; font-size: 30px;">That's
				it, No More Colleges Found for Your Search!</h3>
			<br>
		</center>
</body>
</html>