<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<link rel="stylesheet" href="css/Style.css">
<link rel="stylesheet" href="css/frontStyle.css">
<link rel="stylesheet"
	href="https://unpkg.com/flickity@2/dist/flickity.min.css">
<link rel="stylesheet" href="/path/to/flickity.css" media="screen">
<meta charset="ISO-8859-1">
<script src="https://kit.fontawesome.com/a076d05399.js"></script>
<meta name="viewport"
	content="width=device-width, initial-scale=1, shrink-to-fit=no">
<link rel="stylesheet"
	href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css"
	integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO"
	crossorigin="anonymous">
<link rel="stylesheet" href="css/frontStyle.css">
<link rel="preconnect" href="https://fonts.gstatic.com">
<link
	href="https://fonts.googleapis.com/css2?family=Seaweed+Script&display=swap"
	rel="stylesheet">
<title>StudentHomePage</title>

<!-- The core Firebase JS SDK is always required and must be listed first -->
<script src="https://www.gstatic.com/firebasejs/8.6.1/firebase-app.js"></script>
<script
	src="https://www.gstatic.com/firebasejs/8.6.1/firebase-analytics.js"></script>
<script src="https://www.gstatic.com/firebasejs/8.6.1/firebase-auth.js"></script>

<script>
			var firebaseConfig = {
					apiKey: "${apiKey}",
					authDomain: "${authDomain}",
					projectId: "${projectId}",
					storageBucket: "${storageBucket}",
					messagingSenderId: "${messagingSenderId}",
					appId: "${appId}"
			};

			firebase.initializeApp(firebaseConfig);
			
			var userId="";
			firebase.auth().onAuthStateChanged(function(user) {
			    if (user) {
			      document.getElementById("username").innerHTML = firebase.auth().currentUser.displayName;
			      document.getElementById("userId").innerHTML = firebase.auth().currentUser.uid;
			      userId = firebase.auth().currentUser.uid;
			    } else {
			    	window.location.href = "/";
			    }
			    });
			function myFunction(){
				window.location.replace('/updateStudent/'+userId);
			}
			function getRecommendations(){
				window.location.replace('/getRecommendations/'+userId);
			}
			
	</script>

<script src="js/user.js"></script>

<style>
form {
	width: 380px;
	height: 50px;
	border-radius: 12px;
}
</style>

</head>

<body>

</body>
<div class="container-fluid p-0">
	<nav class="navbar navbar-expand-lg my-navbar">
		<a class="navbar-brand" href="studentHomepage"><img id="logo"
			src="data:image/png;base64,"
			alt="logo"></a>

		<div class="collapse navbar-collapse" id="navbarSupportedContent">
			<ul class="navbar-nav m-auto">

				<li class="nav-item active"><a class="nav-link"
					href="studentHomepage">Home</a></li>
				<li class="nav-item dropdown"><a
					class="nav-link dropdown-toggle" href="#" data-toggle="dropdown">Courses
						Offered</a>
					<ul class="dropdown-menu mega-menu"
						aria-labelledby="navbarDropdown">
						<a class="dropdown-item" href="collegeByBranch/ECE">ECE</a>
						<a class="dropdown-item" href="collegeByBranch/CSE">CSE</a>
						<a class="dropdown-item" href="collegeByBranch/IT">IT</a>
						<a class="dropdown-item" href="collegeByBranch/ME">MECH</a>
						<a class="dropdown-item" href="collegeByBranch/EEE">EEE</a>
					</ul></li>
				<li class="nav-item dropdown"><a
					class="nav-link dropdown-toggle" href="#" data-toggle="dropdown">Rankings</a>
					<ul class="dropdown-menu mega-menu"
						aria-labelledby="navbarDropdown">
						<a class="dropdown-item" href="listAll">Overall Ranks</a>
						<a class="dropdown-item" href="employmentRanking">Ranks
							According Placement</a>
						<a class="dropdown-item" href="researchRanking">Ranks
							According Research</a>
					</ul></li>
				<li class="nav-item active"><a class="nav-link"
					href="javascript:getRecommendations()">Recommended College</a></li>

			</ul>
		</div>

		<div class="profile" onclick="menuToggle();">
			<i class="far fa-user icon-4x"></i>
		</div>
		<div class="menu">

			<ul>

				<li><h3 style="color: black; font-size: 12px;">
						User ID:
						<p id="userId"></p>
					</h3></li>
				<li><i class="far fa-user"></i>
				<button onclick="myFunction()">Update Details</button></li>
				<li><i class="far fa-user"></i>
				<button onclick="resetPassword()">Change Password</button></li>
				<li><i class="far fa-user"></i>
				<button onclick="logoutUser()">Logout</button></li>


			</ul>
		</div>
	</nav>

	<footer class="foot">
		<br> <br>
		<div class="main-content">
			<div class="l box">
				<h2>About Us</h2>
				<div class="footercontent">
					<p>A Web site is developed to display the information regarding
						the college for a student to access the website and consider the
						colleges to pursue their education. The website also asks for
						information such as which path are they interested in, which job
						perspective or higher education.</p>
					<div class="social">
						<a href="#"><span class="fab fa-facebook-f"></span></a> <a
							href="#"><span class="fab fa-twitter"></span></a> <a href="#"><span
							class="fab fa-instagram"></span></a> <a href="#"><span
							class="fab fa-youtube"></span></a>
					</div>
				</div>
			</div>
			<div class="c box">
				<h2>Address</h2>
				<div class="footercontent">
					<div class="place">
						<span class="fas fa-map-marker-alt"></span> <span class="text">No
							5/535, Old Mahabalipuram Road, Okkiam -Thoraipakkam, Chennai
							600096, Tamil Nadu, India.</span>
					</div>

				</div>
			</div>
			<div class="r box">
				<h2>Contact Us</h2>
				<div class="footercontent">
					<div class="phone">
						<span class="fas fa-phone-alt"></span> <span class="text">+91-7547808959</span>
					</div>
					<div class="email">
						<span class="fas fa-envelope"></span> <span class="text">college.ranking@gmail.com</span>
					</div>
				</div>
			</div>
		</div>

	</footer>

</div>
<div class="wrapper">
	<h3>Search and Get your Dream College!</h3>
	<div class="search_box">

		<div class="search_field">
			<form action="/Search">
				<input type="text" class="input"
					placeholder="Search By College, Branch and Location" name="keyword"
					required /> <i class="fas fa-search"></i>
			</form>
		</div>
	</div>
</div>
<div class="left">
	<h3>
		Welcome,
		<p id="username"></p>
		</p>
	</h3>

</div>

<br>
<h1 class="h1-head">Top Rated Colleges:</h1>
<div class="main-carousel"
	data-flickity='{"wrapAround": true, "autoPlay": true}'>

	<div class="carousel-cell">
		<a href="https://www.srmist.edu.in/"><img
			src=" https://th.bing.com/th/id/AMMS_14ffb3dd36b82a62a8fcf60decff2e58?pid=ImgDet&w=316&h=315&rs=1"
			width="150" height="150"></a>
	</div>
	<div class="carousel-cell">
		<a href="https://vit.ac.in/"><img
			src=" https://th.bing.com/th/id/R24f3c0f6b621bcdb8d8c603885405db1?rik=atfP9kGaah636A&riu=http%3a%2f%2fwww.freelogovectors.net%2fwp-content%2fuploads%2f2019%2f07%2fvitlogo.jpg&ehk=z0nHUp%2bAYlg%2b%2b8963WNHVrgDSZKOvvH8bRLoJuVfU7c%3d&risl=&pid=ImgRaw"
			width="150" height="150"></a>
	</div>
	<div class="carousel-cell">
		<a href="https://www.iitb.ac.in/"><img
			src=" https://th.bing.com/th/id/OIP.mjdXGqkx7ACb9nnWyt7dzgHaHR?pid=ImgDet&rs=1"
			width="150" height="150"></a>
	</div>
	<div class="carousel-cell">
		<a href="https://kiit.ac.in/"><img
			src=" https://th.bing.com/th/id/R204fce833de4b2b2c1196521e14a3659?rik=uSfhTWE%2f%2f23Wfw&riu=http%3a%2f%2fupdates.highereducationinindia.com%2fnews_img%2fkiit-university-pg-medical-md-course.jpg&ehk=iUsG6Cb8vALCC6tNP0ewymg2a8sqeXIClDCj6agJZW0%3d&risl=&pid=ImgRaw"
			width="150" height="150"></a>
	</div>
	<div class="carousel-cell">
		<a href="http://www.iitkgp.ac.in/"><img
			src=" https://th.bing.com/th/id/OIP.r5dHql-h9CXCqQmK9-vmtwHaFz?pid=ImgDet&rs=1"
			width="150" height="150"></a>
	</div>
	<div class="carousel-cell">
		<a href="https://bits-pilani.ac.in/"><img
			src=" https://media.glassdoor.com/sqll/735401/bits-pilani-squarelogo-1425302910913.png"
			width="150" height="150"></a>
	</div>
	<div class="carousel-cell">
		<a href="https://amity.edu/"><img
			src=" https://th.bing.com/th/id/Rc7ea4d59e6684825c8ccc74f69b266d6?rik=1zaFRJ3TX7YLOw&riu=http%3a%2f%2fdistanceeducationdelhi.co.in%2fwp-content%2fuploads%2f2016%2f10%2famity_university_logo.jpg&ehk=6VEXS00OslSsP4onJMwov87In4%2fwneSrvlALsZX9%2fBc%3d&risl=&pid=ImgRaw"
			width="150" height="150"></a>
	</div>
	<div class="carousel-cell">
		<a href="https://www.iitm.ac.in/"><img
			src=" https://th.bing.com/th/id/Rc8d6d75221608a698b634aa6c283a853?rik=6XV6ls8XJtUX4g&riu=http%3a%2f%2f2011.igem.org%2fwiki%2fimages%2f4%2f47%2fIITM_Color_Logo.JPG&ehk=l%2fKGMiFxwu86W7mVjdRVMh9Xz9YG0k4h3tVbMCOVusI%3d&risl=&pid=ImgRaw"
			width="150" height="150"></a>
	</div>
	<div class="carousel-cell">
		<a href="https://www.annauniv.edu/"><img
			src=" https://th.bing.com/th/id/Rb7b3c441b399edbecdcd420019af2787?rik=pCH2kDYOqC5Hrw&riu=http%3a%2f%2f1.bp.blogspot.com%2f-7WvRw7QQGTQ%2fUT4UZ6xXKBI%2fAAAAAAAAAqU%2fyJiRRd4CQFQ%2fs1600%2fanna-university-logo.gif&ehk=6EPc1Mg0yOnlyLwFgIEjcxQmQpHjdafpTjbTluB5kyM%3d&risl=&pid=ImgRaw"
			width="150" height="150"></a>
	</div>
	<div class="carousel-cell">
		<a href="https://manipal.edu/"><img
			src=" https://th.bing.com/th/id/OIP.pQ0oHYd7Ry8ENkevsIWoPgAAAA?pid=ImgDet&rs=1"
			width="150" height="150"></a>
	</div>


</div>

<br>

<script src="/path/to/flickity.pkgd.min.js"></script>
<script src="https://unpkg.com/flickity@2/dist/flickity.pkgd.min.js"></script>

<script
	src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
<script
	src="https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.8.1/slick.min.js"></script>

<script src="https://kit.fontawesome.com/b99e675b6e.js"></script>
<script src="https://code.jquery.com/jquery-3.4.1.min.js"></script>
<script>
     function menuToggle(){
       const toggleMenu=document.querySelector('.menu');
       toggleMenu.classList.toggle('active');
     }
   </script>
<script>
		$(document).ready(function(){
			$(".default_option").click(function(){
			  $(".dropdown ul").addClass("active");
			});

			$(".dropdown ul li").click(function(){
			  var text = $(this).text();
			  $(".default_option").text(text);
			  $(".dropdown ul").removeClass("active");
			});
		});
	</script>
<script>document.getElementById("userId").InnerHTML = firebase.auth().currentUser.uid</script>
</body>
</html>